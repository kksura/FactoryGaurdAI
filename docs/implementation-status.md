...
## Current phase
Phase 6 complete. Ready to start Phase 7 (Azure design + IaC: Bicep, AML
job/endpoint YAML, architecture docs, runbooks — all unexecuted-by-design,
...
- 2026-07-19: Phase 6 — MLOps + observability: MLflow tracking (sqlite default + compose server verified), file-backed registry with gated CANDIDATE→CHAMPION lifecycle (live-verified incl. gates rejecting a weak candidate), Prometheus/Grafana observability loop (host API scraped `up` by containerized Prometheus), drift suite with injected-shift validation + OI-7 drift-aware weights (config-gated), retraining workflow with decision files, GB10 benchmark (resolves OI-1/OI-2), first real compose boot (postgres hardening fix D-035).
